...
public class Flock : MonoBehaviour
{
    ...
    void Update()
    {
        foreach (FlockAgent agent in agents)
        {
            ...
            // {
            //     Vector2 desiredVelocity = (move).normalized * maxSpeed * (distance / slowDownRadius);
    ...
    public List<Transform> GetNearbyObjects(FlockAgent agent)
    {
        ...
        foreach (Collider2D one in contextColliders)
        {
            if (one != agent.AgentCollider)
            {
                others.Add(one.transform);
            }
        }
        return others;
    }
